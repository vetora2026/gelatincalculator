...
export const COUNT_PAGES = [
...

  {
    ...
    lede: (v) => `Three sheets is the quantity that sits closest to the packet most American kitchens buy. A Knox envelope holds ${v.packetGrams}g of powder; three gold sheets convert to ${v.gold}g of that same powder, about ${v.packetShortPct}% short of a packet. A gap that size is why so many recipes are written as though the two are interchangeable, and why the substitution usually works well enough that nobody investigates it.`,
    ...
    faqs: (v) => [
      {
        ...
        a: `Close, but not identical. One packet is ${v.packetGrams}g at ${v.knoxBloom} bloom. Three gold sheets convert to ${v.gold}g of that powder — about ${v.packetShortPct}% short of a packet. Three silver sheets convert to ${v.silver}g — closer to the ${v.packetGrams}g packet than gold's ${v.gold}g, but still under it. How far under you land depends entirely on the grade you started from.`,
      },
      {
...

  {
    ...
    measuring: (v) => `Six sheets converts to a powder weight where a kitchen scale is unambiguously the right tool — it is well inside the accurate range of even a cheap one, and the spoon equivalent has grown to roughly ${v.tspApprox} level teaspoons, which is too many to level reliably. Blooming at this scale needs planning rather than improvisation. The powder wants ${v.bloomRatio} times its own weight in cold liquid, about ${v.bloomTbsp} ${v.bloomTbspWord} (${v.bloomMl}ml), and it needs room to swell without the granules touching each other on the way down — sprinkle it across the surface rather than dropping it in one place. Sheets, by comparison, need a bowl large enough that six of them do not stick to one another while they soften, and a firm squeeze before they go in.`,
